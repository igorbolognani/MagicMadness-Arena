  ...
  return <>
    ...
    <div className="combat-controls">
      <div className="skill-row">{([0, 1, 2, 3] as const).map(index => {
        ...
        return <button disabled={locked} key={skill.id} data-testid={`skill-${index + 1}`} className={`skill-button ${controls.heldSkill === index ? "holding" : controls.selectedSkill === index ? "selected-spell" : ""}`} style={{ "--hero-color": hero.color } as React.CSSProperties} title={skill.summary} aria-label={`${skill.name}. Desktop: select, then hold and release left mouse in the arena to cast. Touch: hold and drag to aim; release to cast; drag to cancel zone to cancel.`} onPointerDown={event => controls.beginSkill(event, index)} onPointerMove={controls.dragSkill} onPointerUp={controls.endPointer} onPointerCancel={controls.cancelPointer} onLostPointerCapture={controls.cancelPointer}><span className="skill-key">{["Q", "W", "E", "R"][index]}</span><span className="skill-glyph"><SkillIcon behavior={tuning.behavior} element={hero.element} skillId={skill.id} /></span><small>{locked ? `Lv.${SKILL_UNLOCK_LEVELS[index]}` : cooldown > 0 ? cooldown.toFixed(1) : skill.name}</small>{cooldown > 0 && <span className="cooldown-cover" style={{ height: Math.min(100, cooldown / tuning.cooldown * 100) + "%" }} />}</button>;
      })}</div>
      <div className="utility-row"><button className="utility-button dash-button" onPointerDown={() => controls.fireOneShot("dash")}><strong>⇢</strong><small>DASH · SPACE</small><em>{player.tacticalCooldown > 0 ? player.tacticalCooldown.toFixed(1) : "READY"}</em></button><button className="utility-button" onPointerDown={() => controls.fireOneShot("healthPotion")}><strong>♥</strong><small>HEALTH · 1</small></button><button className="utility-button" onPointerDown={() => controls.fireOneShot("manaPotion")}><strong>◈</strong><small>MANA · 2</small></button></div>
    </div>
  </>;
}
